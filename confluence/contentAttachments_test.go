...
func TestContentAttachmentService_Gets(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			endpoint:           "/rest/api/content/5949392/child/attachment?expand=operations%2Cfile&filename=filename.png&limit=50&mediaType=.png&start=0",
...

		{
			...
			mockFile:           "./mocks/get-content-attachments.json",
			wantHTTPMethod:     http.MethodGet,
			endpoint:           "/rest/api/content/5949392/child/attachment?limit=50&start=0",
			context:            context.Background(),
			wantHTTPCodeReturn: http.StatusOK,
			wantErr:            false,
		},
...
func TestContentAttachmentService_CreateOrUpdate(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			mockFile:           "./mocks/get-content-attachments.json",
			wantHTTPMethod:     http.MethodPut,
			endpoint:           "/rest/api/content/490949439393/child/attachment?status=current",
			context:            context.Background(),
			wantHTTPCodeReturn: http.StatusOK,
			wantErr:            false,
		},
